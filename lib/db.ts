...
// Driving2Develop runs on Prisma's engine-less "client" generator + driver adapters
...
declare global {
  ...
  var __driving2developPrisma: PrismaClient | undefined;
}

const databaseUrl = process.env.DATABASE_URL ?? "file:./dev.db";

export const prisma: PrismaClient =
  global.__driving2developPrisma ?? new PrismaClient({ adapter: createAdapter(databaseUrl) });

if (process.env.NODE_ENV !== "production") {
  global.__driving2developPrisma = prisma;
}
