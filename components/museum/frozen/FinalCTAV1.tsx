...
/**
 * FROZEN HISTORICAL SNAPSHOT - Final CTA V1
 * Exact recovery from commit 8eb300f (components/v4/V4FinalCTA.tsx) - only
 * the H2 headline animated; the subtitle, button, and 3-fact strip were
 * static, before per-item reveal animation was added in a602a21.
 */
  ...
  return (
    <section className="relative overflow-hidden border-t" style={{ borderColor: "var(--v4-border)", background: "var(--v4-bg-inset)" }}>
      ...
      <div className="relative max-w-[1600px] mx-auto px-6 md:px-10 py-24 md:py-32 text-center">
        ...
        <div className="mt-16 pt-10 border-t grid sm:grid-cols-3 gap-8 max-w-4xl mx-auto text-left" style={{ borderColor: "var(--v4-border)" }}>
          {FACTS.map((f) => {
            const Icon = f.icon;
            return (
