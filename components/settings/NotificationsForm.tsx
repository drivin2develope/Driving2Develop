...
export function NotificationsForm() {
  ...
  useEffect(() => {
    try {
      const s = localStorage.getItem("driven2develop_notification_prefs");
  ...
  function toggle(key: string) {
    setToggles((prev) => {
      ...
      try { localStorage.setItem("driven2develop_notification_prefs", JSON.stringify(next)); } catch {}
        ...
        {TOGGLES.map((t) => (
          <div key={t.key} className="flex items-center justify-between gap-4">
            ...
            <button role="switch" aria-checked={!!toggles[t.key]} aria-label={t.label} onClick={() => toggle(t.key)}
              className={`w-11 h-6 rounded-full relative shrink-0 transition-colors ${toggles[t.key] ? "bg-[var(--color-gold)]" : "bg-[var(--color-border-strong)]"}`}>
